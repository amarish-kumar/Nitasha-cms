using NITASA.Core.ComponentModel;
...
namespace NITASA.Core.Plugins
{
    ...
    public class PluginManager
    {
            ...
            using (new WriteLockDisposable(Locker))
            {
                ...
                try
                {
                    ...
                    foreach (var dfd in GetDescriptionFilesAndDescriptors(pluginFolder))
                    {
                        ...
                        if (!pluginDescriptor.SupportedVersions.Contains(NITASAVersion.CurrentVersion, StringComparer.InvariantCultureIgnoreCase))
                        {
        ...
        private static bool IsPackagePluginFolder(DirectoryInfo folder)
        {
            ...
            var temp = folder.FullName.Replace(@"\obj\Debug", "").Split("\\".ToCharArray());
            if (temp.Count() > 0)
            {
                if (temp[temp.Length - 2].Equals("Plugins", StringComparison.InvariantCultureIgnoreCase))
                    return true;
                else return false;
            }
            else return false;
